...
function extractMessages(parentError: any) {
    return _.reduce(
        ...
        (acc: Error[], error) => {
            const path: any[] = _.map(_.tail(error.dataPath.split('/')), (part: string) =>
                part.match(/^\d+$/) ? _.toInteger(part) : part
            );
            const missing = error.params.missingProperty;

            return [...acc, { path: [...path, ...(missing ? [missing] : [])], message: parentError.message }];
        },
        []
    );
}
    ...
    return (originalValues: any) => {
        ...
        if (!validate(values) && validate.errors) {
            const errorsByPath = _.sortBy(
                _.reduce(
                    validate.errors,
                    (acc: Error[], error: Ajv.ErrorObject) => [...acc, ...extractMessages(error)],
                    []
                ),
                ({ path }) => -path.length
            );
