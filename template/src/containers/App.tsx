        ...
        return (
            <Switch>
                ...
                <Route
                    ...
                    exact
                    render={(props) => <SetPassword {...props} service={setPassword} />}
                />
                <Route path="/confirm/:code" exact render={(props) => <SetPassword {...props} service={confirm} />} />{' '}
                <Redirect
                    to={{
                        pathname: '/signin',
                    ...
                    render={() => (
                        <Switch>
                            <Resolver resolve={getUserInfo}>
                                {{
                                    success: ({ data: user }) => (
                                        <SessionContext.Provider value={{ user, role: getUserRole(user) }}>
                                            <RoleSwitch>
                                                {{
                                                    [UserRole.SuperAdminRole]: ({ user }) => (
                                                        <SuperAdminApp logout={resetToken} />
                                                    ),
                                                    default: () => <UnprivilegedApp logout={resetToken} />,
                                                }}
                                            </RoleSwitch>
                                    ...
                                    failure: () => (
                                        <div>
                                            ...
                                            <a onClick={() => resetToken()}>logout</a> and login again.
                                        </div>
                                    ),
                                }}
                            </Resolver>
